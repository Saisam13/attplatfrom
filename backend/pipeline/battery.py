...
from .engine import parse_exim_files, _clean_prices
from .constants import EASE
...
CATEGORY_RULES = [
    ...
    ('Li-ion Battery Scrap', {'keywords': ['LITHIUM ION', 'LITHIUM-ION', 'LI-ION', 'LI ION', 'LIB ', 'NMC', 'LFP', 'LCO', 'LITHIUM BATTER'],
                              'require_scrap': True}),
...
def _percentile_norm(items, key, reverse=False):
    """Rank-normalize items[i][key] to 0-100 (higher = better; reverse flips)."""
    ordered = sorted(items, key=lambda x: x[key], reverse=reverse)
    n = len(ordered)
    for rank, it in enumerate(ordered):
        it[key + '_norm'] = (rank / max(n - 1, 1)) * 100
    return items
...
def run_battery_pipeline(exim_files, log=print, progress=None,
    ...
    def build_items(entities):
        ...
        for name, e in entities.items():
            ...
            for cat, pp in e['prices_by_cat'].items():
                if cat in cat_median and cat_median[cat] > 0 and pp:
            ...
            price_index = (sum(p * w for p, w in zip(idx_parts, idx_weights)) /
                           sum(idx_weights)) if idx_parts else 1.0
            ...
            items.append({
                ...
                'median_price': round(med, 2), 'price_index': round(price_index, 3),
                ...
                '_price_inv': -price_index,  # cheaper => larger
    ...
    def score_items(items, weights):
        for key, _w in weights.items():
            _percentile_norm(items, key)
        for it in items:
            score = sum(w * it[k + '_norm'] for k, w in weights.items())
